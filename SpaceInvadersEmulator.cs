...
namespace SpaceInvaders
{
    public class SpaceInvadersEmulator
    {
        ...
        public async Task RunFrameAsync()
        {
            ...
            // UFO hum is a level signal — start/stop looping based on bit state
        ...
        private void CollectSoundTriggers()
        {
            ...
            if (port3 != _prevPort3)
            {
                byte rising3  = (byte)(port3  & ~_prevPort3); // bits 0→1
                byte falling3 = (byte)(~port3 &  _prevPort3); // bits 1→0
                
                // Bit 0: UFO hum is a sustained level, not a one-shot — loop while high
